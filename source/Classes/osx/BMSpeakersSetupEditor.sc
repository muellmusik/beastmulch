BMSpeakerArrayGUI : BMAbstractGUI {
	...
	*new {| startArray, okayFunc, name, origin |
	...
	makeWindow {| origin |
	   ...
	   window		= Window(name, Rect.new(x, y, 496, 554+31+10), false);
	   window.alwaysOnTop_(true);

	   window.view.decorator = FlowLayout(window.view.bounds);
	   specsList	= ScrollView(window, Rect(0, 0, 160, 508))
				   .hasHorizontalScroller_(false)
				   .hasBorder_(true);
	   numTypes	= BMSpeakerSpec.specs.size;
	   specsList	= VLayoutView(specsList, Rect(4,4,150, numTypes * 24 + 4));

	   BMSpeakerSpec.specs.keys.asArray.sort.do({|spName|
			DragSource(specsList, Rect(0, 0, 150, 20)).string_("   " ++ spName.asString)
				.background_(Color.grey.alpha_(0.2))
				.font_(Font("Helvetica-Bold", 12))
				.beginDragAction_({ BMSpeaker(spec: spName) })
		});

	   speakerListCompView	= CompositeView(window, 160 @ 508)
	   ...
	   speakerList	= ListView(speakerListCompView, 152 @ (508-35))
		...
		speakerList.canReceiveDragHandler = { View.currentDrag.isKindOf(BMSpeaker) };
		speakerList.receiveDragHandler = { var newSpeaker = View.currentDrag;
...
// List's Buttons ---------------------
		...
		// instance variables
		speakerVarsView = CompositeView(window, 160 @ 508).background_(Color.grey.alpha_(0.3));
		...
		instanceVarsBoxes	= [ \name -> "Name", \index -> "Output", \x -> "x", \y -> "y",
			...
			.collectAs({| instVar |
				var speaker, index;
			  	StaticText(speakerVarsView, 54 @ 20)
			  		.string_(instVar.value ++ ":").font_(Font("Helvetica", 12));
			  	if (instVar.key == \name)
			  	   {	instVar.value = TextField(speakerVarsView, 82 @ 20);
			  	   ...
			  	   { instVar.value = NumberBox(speakerVarsView, 82 @ 20);
			  	     instVar.value.action_({| view |
		...
		specText = StaticText(speakerVarsView, 82 @ 20)
			.font_(Font("Helvetica", 12));
		...
		StaticText.new(speakerVarsView, 140 @ 20).string = "Import / Export:";

		importPopUpMenu = PopUpMenu(speakerVarsView, 140 @ 20)
...
BMSubarrayMenuGUI : BMAbstractGUI {
	...
	*new {|parent, outputArray|
	...
	makeWindow {
		...
		StaticText.new(subarraySection, Rect(0,0,180,20)).font_(Font("Helvetica-Bold", 14))
			.string = "Subarrays";
		subarrayView = ListView(subarraySection, (200-8) @ (250 - 26))
		...
		assignSection = CompositeView(window, Rect(0, 0, 200, 281))
			.background_(Color.grey.alpha_(0.3));
		assignSection.decorator = FlowLayout(assignSection.bounds);
		StaticText.new(assignSection, Rect(0,0,180,20)).font_(Font("Helvetica-Bold", 14))
			.string = "Assignments";
		assignView = ListView(assignSection, Rect(0, 0, 200-8, 250-1))
			.canReceiveDragHandler = true;
		assignView.receiveDragHandler = {
			outputArray.addToSubArray(subarrayView.item.asSymbol, View.currentDrag.asSymbol);
			this.updateLists;
		};
		...
		speakerSection = CompositeView(window, Rect(0, 0, 200, 281))
			.background_(Color.grey.alpha_(0.3));
		speakerSection.decorator = FlowLayout(speakerSection.bounds);
		StaticText.new(speakerSection, Rect(0,0,75,20)).font_(Font("Helvetica-Bold", 14))
			.string = "Speakers";
		...
		buttonSection = CompositeView(window, Rect(0, 0, 155, 281));
		...
		StaticText.new(buttonSection, Rect(0,30,155,115))
