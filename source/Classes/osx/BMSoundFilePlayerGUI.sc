BMSoundFilePlayerGUI : BMAbstractGUI {
	...
	*new { |player, name|
	...
	makeWindow {
		...
		clust = VLayoutView(window,Rect(10,10,200,40));
		clust2 = VLayoutView(window,Rect(10,10,200,40));
	     info = StaticText.new(clust, Rect(10,10,150,20));
	     info.font = Font("Helvetica-Bold", 12);
		dur = StaticText.new(clust2, Rect(10,10,150,20));
	...
	makeBigClock {
		bigClock.isNil.if({
			bigClock = Window.new("Big Clock", Rect(600, 800, 800, 180)).alwaysOnTop_(true);
			bigClock.alpha = 0.95;
