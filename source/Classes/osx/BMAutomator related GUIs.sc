...
BMControllerAutomatorGUI : BMAbstractGUI {
	...
	*new {|ca, name, origin|
	...
	makeWindow { |origin|
		...
		backView = CompositeView(scrollView, Rect(0, 20,  width - 10, sfView.bounds.height)).background_(Color.clear);
		...
		curSSTime = StaticText(window, Rect(0, 0, 300, 20))
	...
	makeTimesView {
		...
		timesView = UserView(scrollView, Rect(0, 0, sfView.bounds.width, 20));
	...
	makeEnvView {
		...
		envView = EnvelopeView(backView, Rect(0, 0, sfView.bounds.width, sfView.bounds.height))
			.thumbWidth_(19)
			.thumbHeight_(19)
			.drawLines_(true)
			.drawRects_(true)
...
//				});
	...
	// we use SoundFileView Selections for the snapshot time cursors
	drawSelections {
...
BMSnapShotSliders : BMAbstractGUI {
	...
	*new {|snapshot, parent|
	...
	makeWindow {|parent|
		...
		StaticText(window, Rect(0, 0, window.bounds.width - 132 - widthOffset, 20))
			.font_(font)
			.align_(\right)
...
BMSnapShotSeqConfigGUI : BMAbstractGUI {
	...
	*new {|parent|
	...
	makeWindow {|parent|
		...
		window = Window.new("Snapshot Congig",
			Rect(300, 300, max(128, (numColumns * (toggleWidth + 4)) + 4), 24 * numRows + 28)); // 508
		window.view.decorator = FlowLayout(window.view.bounds);
		window.view.background = Color.rand.alpha_(0.3);
		...
		window.view.decorator.nextLine;
		StaticText(window, Rect(0, 0, window.bounds.width - 132, 20))
			.font_(font)
			.align_(\right)
