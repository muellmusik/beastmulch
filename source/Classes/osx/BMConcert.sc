...
BMConcertGUI  {
	...
	*new {| concertManager, configManager, outputArray, chain, chainFunc,  configsGUIFunc, controllerAutomator, name, origin |
	...
	makeWindow {| origin |
		...
		window 					= Window(name, Rect.new(x, y, 410, 456), false).userCanClose = false;
		windowView				= CompositeView(window, Rect(5, 5, 400, 435)).background_(Color.white.alpha_(0.2));

		concertView 				= CompositeView(windowView, Rect(0, 0, 200, 425));
		concertView.decorator 		= FlowLayout(concertView.bounds, Point(10, 10), Point(10, 10));
		StaticText.new(concertView, 180 @ 20).string_("Pieces").font_(Font("Helvetica-Bold", 12));
		...
		buttonSection 			= CompositeView(windowView, Rect(200, 10, 200 + 56, 425));
		...
		configText	= StaticText.new(buttonSection, 180 @ 20).background_(Color.white)
...
//		buttonSection.decorator.shift(0, 10);
		StaticText.new(buttonSection, 180 @ 20).string = "Import / Export:";

		importPopUpMenu = PopUpMenu(buttonSection, 180 @ 20)
					   .items_([ " ",
	...
	makeSelectConfigurationWindow {| action, origin |
		...
		window 		= Window("Select a Configuration", Rect(origin.x, origin.y, 200 + 20 , 317 + 60), false).userCanClose_(false);
	...
	makeSelectSourceWindow {| event, origin |
		...
		origin		= origin ?? { 490 @ 500 };
		window 		= Window("Sources", Rect(origin.x, origin.y, 260, 110), false).userCanClose_(false);
	...
	makeNewNameWindow {| suggestedName, usedNames, action, origin |
		...
		origin		= origin ?? { 490 @ 500 };
		window 		= Window("Select a Name", Rect(origin.x, origin.y, 260, 110), false).userCanClose_(false);
		window.view.decorator = FlowLayout(window.view.bounds, Point(10, 10), Point(10, 10));

		StaticText(window, 50 @ 20).string = "Name:";

		pieceNameField	= TextField(window, 180 @ 20).string_(suggestedName.asString);
		window.view.decorator.shift(0, 30);
