BMSpeakerListVisualiser : BMAbstractGUI {
	...
	*new {|speakerList| ^super.new.init(speakerList).makeWindow }
	...
	makeWindow {
		...
		window = Window("Speakers", rect = Rect(400,200, 800, 800 * [hiY, lowY].abs.maxItem)).front;

		height = Slider(window, Rect(0,0,rect.width, 20));
		distance = Slider(window, Rect(0,20,rect.width, 20));

		qcView = QuartzComposerView(window, rect.resizeBy(0,-40).moveTo(0,40));
