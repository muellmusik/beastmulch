...
BMConfigurationsGUI : BMAbstractGUI {
	...
	*new {|chain, configurations, concertManager, name, origin|
	...
	makeWindow {|origin|
		...
		window = Window(name, Rect.new(x, y, 450 + configViewWidth + 27, 450 + 6), false);
		window.view.decorator = FlowLayout(window.view.bounds);
		window.userCanClose_(false);


		configView			= CompositeView(window, configViewWidth @ 435);
		configView.decorator 	= FlowLayout(configView.bounds, Point(5, 5), Point(5, 5));
		configView.background	= Color.white.alpha_(0.2);
		StaticText(configView, 180 @ 20).string_("Configurations").font_(Font("Helvetica-Bold", 12));
		configView.decorator.shift(0, 5);
		configListView		= ListView(configView, 200 @ 373).canReceiveDragHandler = false;
		...
		chainView				= ScrollView(window, 465 @ 435).hasBorder_(false);
		if(width <= 465, { chainView.hasHorizontalScroller = false });
		chainView 			= CompositeView(chainView, Rect(0, 0, width, max(450, rows * 20)));
		chainView.background 	= Color.white.alpha_(0.2);
		chainView 			= UserView(chainView, Rect(0, 0, width, max(450, rows * 20)));
...
/*			if (change == \freed)
	...
	makeNewConfigWindow {| method, origin |
		...
		window 			= Window(method + "Configuration", Rect(origin.x, origin.y, 260, 110), false).userCanClose_(false);
		window.view.decorator = FlowLayout(window.view.bounds, Point(10, 10), Point(10, 10));

		StaticText(window, 50 @ 20).string = "Name:";

		pieceNameField	= TextField(window, 180 @ 20);
