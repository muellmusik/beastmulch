...
BMAutoBalancer {
	...
	*run {|speakerList, okayFunc, server, in = 0, onlyFullRange = true|
	...
	sendDef {|server|
		//SynthDef("BMAutoBalance", {|out, in = 1, amp = 0.3|
//			var max;
//			Out.ar(out, PinkNoise.ar(amp) * EnvGen.kr(Env.linen, timeScale: 0.3, doneAction: 2));
//			max = RunningMax.kr(Amplitude.kr(AudioIn.ar(in)));
//			SharedOut.kr(0, max);
//		}).send(server);
